...
void group_3::SetCurrentGroup(int j,screen *c_frame=0, screen *l_frame=0)
{
	for(int i=0;i<G_3_AMOUNT;i++)
	{	//	R	G	B	S Efx ctrl time pos
		pixel[i].red = (uint8_t)pgm_read_byte(&(G_3[j][i][0]));
		pixel[i].green = (uint8_t)pgm_read_byte(&(G_3[j][i][1]));
		pixel[i].blue = (uint8_t)pgm_read_byte(&(G_3[j][i][2]));
		speed = (uint8_t)pgm_read_byte(&(G_3[j][i][3]));
		Efx = (uint8_t)pgm_read_byte(&(G_3[j][i][4]));
		control = (uint8_t)pgm_read_byte(&(G_3[j][i][5]));
		time = (uint8_t)pgm_read_byte(&(G_3[j][i][6]));
		position[i] = (uint8_t)pgm_read_byte(&(G_3[j][i][7]));
		if(c_frame)
		{
...
void group_3::handler(screen *l_frame, screen *c_frame){
			...
			case SMOTH_BLINK:
				...
				}else if (control == 1) //smothing up
				{
				  ...
				  (c_frame->pixel[pos].blue < l_frame->pixel[pos].blue)?
						  ...
						  c_frame->pixel[pos].blue;
				  for(int pix = 0;pix<G_3_AMOUNT;pix++)
				  {
