...
void group_1::SetCurrentGroup(int j,screen *c_frame=0, screen *l_frame=0)
{
	for(int i=0;i<G_1_AMOUNT;i++)
	{	//	R	G	B	S Efx ctrl time pos
		pixel[i].red = (uint8_t)pgm_read_byte(&(G_1[j][i][0]));
		pixel[i].green = (uint8_t)pgm_read_byte(&(G_1[j][i][1]));
		pixel[i].blue = (uint8_t)pgm_read_byte(&(G_1[j][i][2]));
		speed = (uint8_t)pgm_read_byte(&(G_1[j][i][3]));
		Efx = (uint8_t)pgm_read_byte(&(G_1[j][i][4]));
		control = (uint8_t)pgm_read_byte(&(G_1[j][i][5]));
		time = (uint8_t)pgm_read_byte(&(G_1[j][i][6]));
		position[i] = (uint8_t)pgm_read_byte(&(G_1[j][i][7]));
		if(c_frame)
		{
...
void group_1::handler(screen *l_frame, screen *c_frame){
					...
					int pos = position[pixel];
							...
							}else if (control == 1) //smothing up
							{
							  ...
							  (c_frame->pixel[pos].blue < l_frame->pixel[pos].blue)?
									  ...
									  c_frame->pixel[pos].blue;
							  for(int pix = 0;pix<G_1_AMOUNT;pix++)
							  {
								  if((c_frame->pixel[pix].red == l_frame->pixel[pix].red) &&
										  (c_frame->pixel[pix].green == l_frame->pixel[pix].green) &&
											  (c_frame->pixel[pix].blue == l_frame->pixel[pix].blue) ){
									  control = 2;
								  }else
								  {
									  control = 1;
